...
验证量子热 n,J 采样（NACTA=2）在 MORSE 双原子 H₂（2×1.008 u）上的统计正确性：
振动量子数服从几何分布，转动量子数服从带核自旋权重的 Boltzmann 分布，且
(n,J)→能量映射逐位确定。
...
> 图注：左 = 各轨迹 EVIBA+EROTA（3.5-25.4 kcal/mol 波动，n,J 热分布的直接体现），
> 右 = 逐轨迹采样过程（stdout `CHOSEN:`，10 条轨迹）。判定依据：n 的经验分布与
> 几何分布 q=0.1876 的 KS Dmax=0.0176 ≤ 3/√N=0.0949（直方图 n=0..5 为
> 830/141/24/5/0/0，几何衰减）；J 分布 KS Dmax=0.0129，偶/奇 J 计数 287/713 与
> 正/仲权重 0.25/0.75 吻合；(n,J)→ENJA 逐位确定，范围 [0.1381, 1.2230] ⊂ (0, De)。
