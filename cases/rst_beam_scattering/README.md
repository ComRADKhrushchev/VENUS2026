# rst_beam_scattering — RST C/Au(111) 束流散射多轨迹

> 运行：`cd cases/rst_beam_scattering && ../../venus_test.e`（主输入文件 `input_qct.txt`）

## 1. 目的

在 RST 势下以 Au(111) 板块复现束流散射流程：法向入射、瞄准点在斜胞内均匀
抽取、固定相对平动能，检验 C 原子的散射/吸附动力学（高度演化、最近邻距离
物理范围、能量守恒）。

## 2. 理论与体系

RST 势（C-Au 相互作用）：
- 成对 1D-NN（tanh 激活，GS 态 NH_surf=12/NH_inner=8，63 参数，nn_weights_rst.txt）。
- cosine 平滑截断（r_cut=6、d_cut=0.5）+ 排斥核 A=10/B=3，吸附质受力。

板块（BVK 弹性 Au(111) slab，参数取 Analytic_Potential.txt）：
- cutoff 6.0 / a_lat 2.95 / alpha 0.5 / beta 0.3 / gamma 0.1。
- GENERATE 生成 Au₁₄₄，BOXLX=20.65、BOXLY=18.2786、SKEW=90°。

初始条件与数值设置：
- QZA_EQ=0,0,0.5 → 初始 C 高度 z=6.5 Å（渐近区，势能≈0），vz<0 朝表面。
- NS=30000（DT=0.01=0.1 fs → 3000 fs）、NIP=50、ISEED=20260821、GWRITE_LEVEL=1
 （fort.10NN 头含 E0/T/H、Q(C)、r_min）。

## 3. 方法与流程

1. 运行 NT=20 条轨迹（run_full.log）。
2. plot_fig.py 逐行 split 解析 fort.1001-1020 叠绘 20 条 z(t) 与 r_min(t)
   → fig_scattering.png（dpi=150，ps 轴）。
3. 终止分类：输出块数达到最大值（NS/NIP+1=601）说明轨迹跑满全程 → 吸附；块数不足说明散射提前终止。

## 4. 核心验证

见 `fig_scattering.png`。

> 图注：上 = 20 条轨迹 C 高度 z(t)（红虚线=顶位阱 z_eq=2.90 Å；红色轨迹=吸附，
> 其余=散射），下 = C–Au 最近距离 r_min(t)。判定依据：散射 19/20、吸附 1/20
>（吸附轨迹跑满全程，散射轨迹在 z 达 RMAX=6 Å 判定散射完成提前终止）；
> 入射能 0.63 eV，散射轨迹将大部分能量返还（弹性 slab 声子通道有限吸能）；
> z_min 全域 1.17-1.95 Å；H 守恒（渐近区 E0=T=0.63 eV）。
