...
验证玻尔兹曼振动采样（`INIT_SAMPLING_A=BOLTZMANN-VIB`）：每个简正模量子数
服从几何分布，且采样平均能量与给定振动温度的定义一致。

## 2. 理论与体系

体系与势：
- 等边 H₃（NATOMS=3，全为 1.008 u），TEST 势 LEPS。
- 等边极小处谱：简并 E′ 对 1013.5 cm⁻¹ ×2 与呼吸模 1803.8 cm⁻¹（stdout 频谱表）。

抽样理论（`THRMAN` 实现）：
- GAMA=−ln(U) 服从指数分布，n=trunc(GAMA/DUM)，DUM=1.43878·ν/T=hν/k_BT。
- 由此 P(n)=(1−q)qⁿ，q=exp(−hν/k_BT)。
- T=2000 K 时 q(E′)=0.482（双峰简并）、q(呼吸)=0.273，⟨n⟩=q/(1−q)=0.93/0.93/0.38。
- 理论期望 E[EVIBA]=12.815 kcal/mol。
## 3. 方法与流程
