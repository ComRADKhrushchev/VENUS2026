...
!2017/2/5
      ...
      FACTOR1=413.4D0                                         !-->TIME^-
      FACTOR2=413.4D0                                         !-->CONVER
      FCG=FCG*FACTOR1
      DO I=1,3
      WS1(I)=WS1(I)*FACTOR2
      WS2(I)=WS1(I)**2
      ...
      use venus_params
      use venus_data, only: C1, NATOMA, W, Q, P, WEFF, WG1, WGS1, &
                            ATIME, FCG, GSW, COEFA, COEFB, &
                            WS2, WG2, WGS2
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      integer :: ISEED
      ...
      P(K)=FACTOR*G4
      ENDDO
!      FCG=0D0   !REMOVE DAMPING FORCE AND WHITE NOISE FOR CHECKING THE
      ...
      use venus_params
      use venus_data, only: C1, NATOMA, W, Q, P, PDOT, GN, WEFF, WG1, WGS1, &
                            ATIME, FCG, GSW, WS2, WG2, WGS2, NATOMS, NI, &
                            TEMP, NC
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      integer :: IDUM
      DIMENSION FT(NDA3yf)
